use crate::components::{PixelCamera, ViewportCamera, ViewportImage};
...
pub(crate) fn update_viewport_size(
    pixel_cameras: Query<(&PixelCamera, &Children, &RenderTarget)>,
    ...
    for (pixel_camera, children, pixel_target) in &pixel_cameras {
        let (mut proj, viewport_target) = viewport_cameras
            .get_mut(children[0])
...
pub(crate) fn snap_camera_position(
    mut cameras: Query<(&mut GlobalTransform, &Children, &PixelCamera)>,
    mut canvas_transforms: Query<
        (&mut Transform, &mut GlobalTransform),
        (With<ViewportImage>, Without<PixelCamera>),
    >,
) {
    for (mut camera_global, children, pixel_camera) in &mut cameras {
        ...
        let snapped_local_translation = Vec3::new(
            local_translation.x.round(),
            local_translation.y.round(),
            local_translation.z,
        );
...
pub(crate) fn sync_camera_fields(
    pixel_cameras: Query<(Ref<PixelCamera>, &Children)>,
    mut viewport_cameras: Query<(&mut RenderLayers, &mut Camera)>,
) {
    for (pixel_camera, children) in pixel_cameras {
        let (mut viewport_camera_layers, mut viewport_camera) = viewport_cameras
            .get_mut(children[0])
            .expect("child 0 is the viewport camera as per the on_add hook");
        ...
        if pixel_camera.is_changed() {
            *viewport_camera_layers = pixel_camera.viewport_layers.clone();
            ...
            viewport_camera.clear_color = pixel_camera.viewport_size.clear_color();
        }
    }
}
